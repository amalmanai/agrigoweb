...
class GoogleAuthenticator extends OAuth2Authenticator
{
    ...
    public function __construct(
        ...
        private string $userQrCodesDirectory
    ) {
    }
        ...
        return new SelfValidatingPassport(
            new UserBadge($accessToken->getToken(), function () use ($accessToken, $client) {
                ...
                if (!$user) {
                    ...
                    $user = new User();
                    $user->setEmailUser((string) $email);
                    ...
                    // Generate QR Code
                    $qrCodeDirectory = $this->userQrCodesDirectory;
                    if (!is_dir($qrCodeDirectory)) {
                        mkdir($qrCodeDirectory, 0777, true);
                    }
                    $qrCodePath = $qrCodeDirectory . '\user_' . $user->getIdUser() . '_' . $user->getEmailUser() . '.png';
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName): ?Response
    {
        ...
        // Always redirect to home after Google authentication (ignore target path)
